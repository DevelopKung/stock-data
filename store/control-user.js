...
export const actions = {
  async loadData({ state, commit, dispatch }) {
    try {
      const url = routeAPI.member_user.lists;
  ...
  async loadInfo({ state, commit, dispatch }, id) {
    try {
      const url = routeAPI.member_user.info.replace('{:id}', id);
      let config = { headers: { Authorization: this.$auth.getToken('local') } }
      const res = await this.$axios.$get(url, config);
      return res
    } catch (error) {
  ...
  async create({ commit, dispatch }, form) {
    try {
      const url = routeAPI.member_user.create;
      let config = { headers: { Authorization: this.$auth.getToken('local') } }
  ...
  async update({ commit, dispatch }, { id, form }) {
    try {
      const url = routeAPI.member_user.update.replace('{:id}', id);
      let config = { headers: { Authorization: this.$auth.getToken('local') } }
  ...
  async delete({ commit, dispatch }, id) {
    try {
      const url = routeAPI.member_user.delete.replace('{:id}', id);
      let config = { headers: { Authorization: this.$auth.getToken('local') } }
